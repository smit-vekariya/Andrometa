...
def google_auth_url(request):
    try:
        flow = Flow.from_client_secrets_file(
            'S:\client_secret_463613713681-a17d76fgstuknb86rlkh5vpb134o4fdu.apps.googleusercontent.com.json',
        ...
        cache.set(f"google_cv_{state}", flow.code_verifier, timeout=300)  # 5 min TTL
...
def google_callback(request):
    try:
        ...
        code_verifier = cache.get(f"google_cv_{state}")
