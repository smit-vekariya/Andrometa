...
def convert_jpeg_to_png(file):
    """
    Convert a JPEG image to PNG format using Pillow.
...
def convert_png_to_jpeg(file):
    """
    Convert a PNG image to JPEG format using Pillow.
...
def convert_image_to_webp(file):
    """
    Convert a JPEG or PNG image to WEBP format using Pillow.
    Returns a tuple of (file_bytes, output_filename, content_type).
    """
    original_name = os.path.splitext(file.name)[0]
...
def convert_webp_to_jpeg(file):
    """
    Convert a WEBP image to JPEG format using Pillow.
...
def convert_webp_to_png(file):
    """
    Convert a WEBP image to PNG format using Pillow.
    Returns a tuple of (file_bytes, output_filename, content_type).
    """
    original_name = os.path.splitext(file.name)[0]
    output_filename = f"{original_name}.png"
